    ...
    (html
        [:div.container                 ;TODO use page-html
         [:div.header
          ...
          [:div#app]
          [:span "Generated by " [:a {:href alzabo-link} "Alzabo"]]]
         (when-let [exp (config/config :explanation)]
  ...
  (let [categories (or categories (config/config :categories)) ;TODO kludge
    ...
    (with-open [wrtr (clojure.java.io/writer dot-file)]
      (binding [*out* wrtr]
        ...
        (println (attributes
                  {:rankdir (case (config/config :orientation)
                              :horizontal "LR"
                              :vertical "TB")
                   :size (str (config/config :height) "," (config/config :width))
                   }
                  ";"))
